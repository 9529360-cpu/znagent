...
function ensurePythonRuntime() {
  ...
  const runtimeHash = sha256Files([
    path.join(RUNTIME_PROJECT, 'pyproject.toml')
  ])
  const stampedHash = readTextIfExists(PYTHON_STAMP)
  const runtimeImport = capture(pythonPath, [
    '-c',
    "import importlib.util; import sys; sys.exit(0 if importlib.util.find_spec('zn_agent.resident') else 1)"
  ])
  const playwrightImport = capture(pythonPath, [
    '-c',
  ...
  const installRequired = stampedHash !== runtimeHash || runtimeImport === null || playwrightImport === null || !browserReady
...
function doctor() {
  ...
  ok = checkDoctorItem(
    'Node dependencies',
    npmReady && probeNodeRuntime(),
    npmReady && probeNodeRuntime() ? 'locked install and Electron/esbuild are ready' : 'setup missing or stale',
    'Run npm run setup.'
  ) && ok
  ...
  console.log(`[INFO] Source-development ZN home — ${devHome}`)
...
function setup() {
  ...
  console.log(`[zn-dev] ready: Python ${python.version}, isolated ZN home ${devHome}`)
  return python
}
...
async function runDev() {
  ...
  const env = {
    ...process.env,
    ZN_RESIDENT_PYTHON: python.pythonPath,
    ZN_AGENT_HOME: devHome,
    PLAYWRIGHT_BROWSERS_PATH: PLAYWRIGHT_ROOT
  }
  console.log(`[zn-dev] launching ZN Desktop with isolated state at ${devHome}`)
