...
- implementation-status synchronization: `15f1c893ee45c793a54da6ca339ae062d64d1f81`
...
### 1. New full-Kernel regression identified separately from the known path defect

At earlier head `59af7bd050595682e3c0354d67d73187c6db7829`, ordinary Kernel CI reported 581 tests / 29 errors / 5 skipped and included:

```text
AttributeError: 'FocusedModernTextResidentRuntime' object has no attribute 'completion_observations'
```

That new failure was not classified as the old DOS-8.3 problem. It caused cascading Intentional/Will/transfer failures because post-completion observation could not run.

### 2. Completion-observation ownership corrected for both legitimate resident births

The ordinary birth remains:
...
The richer birth remains intentionally separate:
...
Calling the base constructor would temporarily create the wrong LifeCore and is not an acceptable compatibility fix.

`EmbodiedResidentRuntime` now installs the journal after the richer Life is awake and repairs pending Life observations from durable `EventOutcome` truth. Direct `EmbodiedResidentRuntime`, `IntentionalResidentRuntime`, and later Transfer/World/Focused descendants therefore inherit the correct organ.
...
ef4d68b363fb381ac74ca83d6623a024651575a7  product resident status projection
...
The `a2428564` full-file edit was diff-reviewed and an accidental removal of explanatory comments/docstrings was immediately restored by `a395897e`. Net behavioral change in `embodied_resident.py` is only journal import + installation/repair after richer Life wake.

### 3. Sanitized health is reachable through product status and RPC
...
Regression coverage verifies healthy and pending states and asserts raw exception text and `last_error` are absent. `ResidentRpcServer` status regression verifies the same sanitized object passes through the existing status RPC without becoming daemon/renderer-owned state.
...
`.github/workflows/zn-work-recovery-e2e.yml` now triggers on and compiles `embodied_resident.py`, includes `test_completion_observation_embodied.py`, and directly completes events through both `EmbodiedResidentRuntime` and `IntentionalResidentRuntime`.

### 5. Nervous post-completion audit reached the actual transaction boundary
...
`NativeWill` already performs durable outcome reconciliation on construction and remains the Will authority.

`PersistentNervousSystem.perceive()` is not safe for blind replay. A single perception currently commits trace, neural links, and affect/nervous state independently. A repeated matching perception intentionally increments repetitions and reinforces trace/link/affect state again.

A receipt around the current call would therefore be false safety: a crash after partial plasticity could either leave incomplete perception or cause duplicate reinforcement on retry. No nervous exactly-once claim and no naive repair queue were added.
...
Focused modules include the existing Work progress/restart/side-effect/cancellation suite plus `test_completion_observation_recovery` and `test_completion_observation_embodied`.
...
- `FocusedModernTextResidentRuntime` currently performs a redundant second construction of the same completion-observation journal after the Embodied root already installed it. It is functionally harmless and not a correctness blocker, but remains cleanup.
...
partial nervous writes cannot masquerade as complete perception
...
Remove the redundant second `CompletionObservationJournal` initialization from `FocusedModernTextResidentRuntime` while retaining its product `status()` projection.
