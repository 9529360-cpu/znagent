...
export async function verifyPackagedZnRuntime(runtimeRoot, { version, commit }) {
  ...
  const chromeDevtoolsMcpRoot = resolveInside(
    runtimeRoot,
    manifest.chrome_devtools_mcp_runtime,
    'chrome_devtools_mcp_runtime'
  )
  if (manifest.chrome_devtools_mcp_version !== CHROME_DEVTOOLS_MCP_VERSION) {
    throw new Error(
      `packaged Chrome DevTools MCP version mismatch: expected ${CHROME_DEVTOOLS_MCP_VERSION}, got ${manifest.chrome_devtools_mcp_version}`
    )
  }
  ...
  await requireDirectory(chromeDevtoolsMcpRoot, 'Chrome DevTools MCP runtime root')
  ...
  await requireFile(chromeDevtoolsMcpManifestPath, 'Chrome DevTools MCP package manifest')
  await requireFile(chromeDevtoolsMcpServer, 'Chrome DevTools MCP server')
  const chromeDevtoolsMcpManifest = JSON.parse(await fs.readFile(chromeDevtoolsMcpManifestPath, 'utf8'))
  if (
    chromeDevtoolsMcpManifest?.name !== 'chrome-devtools-mcp'
    || chromeDevtoolsMcpManifest?.version !== CHROME_DEVTOOLS_MCP_VERSION
  ) {
    throw new Error(
      `packaged Chrome DevTools MCP package mismatch: expected chrome-devtools-mcp@${CHROME_DEVTOOLS_MCP_VERSION}`
    )
  }
  ...
  try {
    await run(runtime.python, ['-I', '-c', PACKAGED_RUNTIME_SMOKE], {
      ...
      env: {
        ...
        ZN_CHROME_DEVTOOLS_MCP_ROOT: runtime.chromeDevtoolsMcpRoot,
