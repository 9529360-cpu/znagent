...
test('scheduled task pins the formal resident to the installed runtime and home', () => {
  ...
  const xml = taskXml({
    command: python,
    args: `-m zn_agent.resident --home "${znHome}"`
  })

  assert.deepEqual(
    validateScheduledTaskXml(xml, { znHome, expectedRuntimeId: runtimeId }),
    { command: python, args: `-m zn_agent.resident --home "${znHome}"` }
  )
})
...
test('scheduled task rejects a Python command outside the installed runtime', () => {
  ...
  const znHome = path.join(os.tmpdir(), 'ZN Home')
  const xml = taskXml({
    command: path.join(os.tmpdir(), 'foreign', 'python.exe'),
    args: `-m zn_agent.resident --home "${znHome}"`
  })

  assert.throws(
    () => validateScheduledTaskXml(xml, { znHome, expectedRuntimeId: runtimeId }),
    /outside installed runtime/
  )
})
...
test('scheduled task rejects legacy or non-formal resident entrypoints', () => {
  ...
  const xml = taskXml({
    command: python,
    args: `-m zn_agent.core.resident_server --home "${znHome}"`
  })

  assert.throws(
    () => validateScheduledTaskXml(xml, { znHome, expectedRuntimeId: runtimeId }),
    /does not launch formal resident/
  )
})
...
test('scheduled task rejects a different ZN home even with the right runtime command', () => {
  ...
  const xml = taskXml({
    command: python,
    args: `-m zn_agent.resident --home "${path.join(os.tmpdir(), 'Other Home')}"`
  })
