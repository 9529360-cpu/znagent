...
test('Work restore points expose only explicit missing-target no-replace application', () => {
  ...
  assert.match(workbench, /details\.restorePoints/)
  assert.match(workbench, /ZnMissingRestoreControls/)
  assert.match(workbench, /details\.restoreSafety/)
  ...
  assert.match(controls, /restore\.controls\.prepare/)
  assert.match(controls, /restore\.controls\.approve/)
  assert.match(controls, /restore\.controls\.description/)
  assert.match(localization, /'restore\.controls\.prepare': 'Prepare restore'/)
  assert.match(localization, /'restore\.controls\.prepare': '准备恢复'/)
  assert.match(localization, /never overwrites a target that exists again/)
  assert.match(localization, /如果目标再次出现，绝不会覆盖它/)
