...
test('ZN Electron main owns window and protocol lifecycle without inherited desktop main', () => {
  ...
  assert.match(source, /scheme:\s*ZN_APP_SCHEME/)
  assert.match(source, /standard:\s*true/)
  assert.match(source, /secure:\s*true/)
  assert.match(source, /protocol\.handle\(ZN_APP_SCHEME/)
  assert.match(source, /ZN_APP_FILES\.get\(url\.pathname\)/)
  assert.match(source, /window\.loadURL\(ZN_APP_URL\)/)
...
test('active ZN renderer root is content-first and independent of inherited shell', () => {
  ...
  assert.match(workbench, /useZnWorkReconnection/)
  assert.match(workbench, /ZnExecutionMode/)
  assert.match(workbench, /zn\.desktop\.execution-mode\.v1/)
  assert.match(workbench, /composer\.modeAria/)
  assert.match(workbench, /executionMode === 'ask'/)
  assert.match(workbench, /startZnWork\(threadId, task, executionMode\)/)
  assert.match(residentClient, /payload:\s*\{ execution_mode: executionMode \}/)
  assert.match(localization, /'composer\.ask': 'Ask'/)
  assert.match(localization, /'composer\.agent': 'Agent'/)
  assert.match(residentClient, /loadZnWorkProgress/)
  ...
  assert.match(workbench, /className="zn-stop-button"/)
  assert.doesNotMatch(workbench, /zn-send-stop/)
  assert.match(workbench, /className="zn-send"/)
  assert.match(workbench, /disabled=\{submissionBusy \|\| stoppingWork \|\| !draft\.trim\(\)\}/)
  assert.doesNotMatch(workbench, /while \(!current\.terminal/)
...
test('running Work progress and Stop control come from resident state', () => {
  ...
  assert.match(workbench, /className="zn-stop-button"/)
  assert.match(workbench, /const workActive = useZnWorkReconnection/)
  assert.match(workbench, /if \(!task \|\| submissionBusy \|\| stoppingWork \|\| !activeThread\) return/)
  assert.match(workbench, /disabled=\{submissionBusy \|\| stoppingWork \|\| !draft\.trim\(\)\}/)
  assert.doesNotMatch(workbench, /zn-send-stop/)
  assert.doesNotMatch(workbench, /while \(!current\.terminal/)
  assert.doesNotMatch(
