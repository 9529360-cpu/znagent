...
test('ZN preload exposes only the ZN bridge and does not import inherited preload', () => {
  ...
  assert.match(source, /zn:resident:turn-submit/)
...
test('active ZN renderer root is content-first and independent of inherited shell', () => {
  ...
  assert.match(workbench, /submitZnTurn/)
  assert.doesNotMatch(workbench, /await startZnWork/)
  ...
  assert.match(localization, /Talk to the model naturally/)
  ...
  assert.match(residentClient, /resident\.turnSubmit/)
