...
test('hidden Work completion uses privacy-safe resident attention instead of another Work owner', () => {
  const main = read('electron/zn-main.ts')
  const preload = read('electron/zn-preload.ts')
  const env = read('src/zn/desktop-env.d.ts')
  const workbench = read('src/zn/workbench.tsx')

  assert.match(main, /zn:shell:work-attention/)
  assert.match(main, /displayBalloon\(/)
  assert.match(main, /respectQuietTime:\s*true/)
  assert.match(main, /noSound:\s*true/)
  assert.match(main, /balloon-click/)
  assert.match(main, /event\.sender !== window\.webContents/)
  assert.match(main, /eventId\.length > 256/)
  assert.match(main, /Work completed\. Open ZN to review the result\./)
  assert.doesNotMatch(main, /payload\.task|payload\.prompt|taskText|promptText/)
  assert.match(preload, /reportWorkAttention/)
  assert.match(env, /reportWorkAttention/)
  assert.match(workbench, /function workAttentionState\(/)
  assert.match(workbench, /recovery\?\.replayBlocked/)
  assert.match(workbench, /presentWorkProgress\(current\)/)
  assert.match(workbench, /presentWorkProgress\(null\)/)

  const catchStart = workbench.indexOf('} catch (error) {', workbench.indexOf('const submit = useCallback'))
  const catchEnd = workbench.indexOf('} finally {', catchStart)
  assert.ok(catchStart >= 0)
  assert.ok(catchEnd > catchStart)
  assert.match(workbench.slice(catchStart, catchEnd), /setWorkProgress\(null\)/)
  assert.doesNotMatch(workbench.slice(catchStart, catchEnd), /presentWorkProgress\(null\)/)
})
test('ZN global invocation is Windows-only, best-effort, and reuses the resident surface', () => {
  ...
  assert.ok(source.indexOf('windowsResidentSurface.show()') < source.indexOf('if (mode) setZnWindowMode(window, mode)'))
...
test('running Work progress and uncertain cancellation come from resident state', () => {
  ...
  assert.match(workbench, /Outside-world effect is uncertain/)
  assert.match(workbench, /Stop work/)
