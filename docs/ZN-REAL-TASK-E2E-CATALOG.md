...
| E2E-12 | CLOSED representative path | One exact attached-workspace DOCX with 1–3 explicit `【待补充：...】` placeholders can be completed from successfully extracted public-Web evidence through the existing Product Resident / Work / Body. Search snippets are discovery only; instruction-like retrieved content is screened before synthesis; every replacement requires at least two independent prompt-safe extracted sources with exact-quote/anchor support; deterministic source-anchor consistency blocks model-omitted conflicts before mutation. Unknown/conflict/unsafe evidence/drift/collision fail closed with zero completed copy. Successful output is a new DOCX, source remains unchanged, and fresh reopen verifies target, identity, non-target text and run formatting. Not arbitrary DOCX completion, arbitrary Deep Research, general prompt-injection defense or Word complete. |
...
| E2E-33 | CLOSED representative path | Same durable Work can continue after restart with fresh current evidence and completed historical effects are not blindly replayed. Broader cross-day breadth remains open. |
...
Representative closure (2026-09-11): E2E-12 is now **CLOSED representative path / VERIFIED NARROW** through the existing active Product Resident / Work / Body. The first version is deliberately narrow: one explicit real `document_path` under the exact Root Work attached workspace, one ordinary DOCX, and 1–3 explicit `【待补充：...】` placeholders in supported body/simple-table paragraphs. Stable target IDs are bound deterministically to location/span/placeholder/paragraph identity; run-aware mutation preserves source bytes, non-target text, run topology and run formatting, writes only a new `-completed.docx`, and fresh-reopens the output before Root completion.

External information reuses the existing configured `WebResource` and the Research evidence primitives. Search results/snippets/provider answers are discovery only; only successfully extracted source-document observations are evidence. At least two independent readable sources are required. Retrieved source content is treated as untrusted data: instruction-like prompt-injection content is deterministically rejected from the synthesis evidence pack and retained as rejection provenance. Model synthesis is only a candidate. Before any mutation, every replacement must be supported by at least two independent prompt-safe extracted sources with exact source IDs/quotes and matching factual anchors, and a deterministic target-relevant price/date/percentage consistency gate scans readable sources so a model cannot permit mutation merely by returning `conflicts: []` while contradictory source anchors exist.

Unknown required facts, verified or locally detected source conflict, unsafe retrieved content that leaves insufficient prompt-safe support, invalid schema/supports, unsupported numeric/date evidence, source drift, target drift, malformed/unsupported DOCX structure, and destination collision all fail closed with zero completed-copy mutation. The two merge blockers were covered by counterexample tests: an omitted-conflict case where the model returned a valid replacement plus `conflicts: []` despite a contradictory extracted price, and an allowed-schema prompt-injection case where malicious retrieved text induced an otherwise schema-valid replacement; both are blocked before `write_docx_completion_copy`.

PR #250 implementation exact-head `65d5ec40f705f4e6e4c624b4a3287f27a949d8dc` was accepted before documentation synchronization. Exact-head workflows were all final success: Document Research Completion E2E #5, ZN CI #1804, Research and Information Work E2E #34, Local Documents and Spreadsheet Work E2E #17, ZN Windows Interactive Desktop E2E #378 and Memory and Learned Behavior E2E #61. E2E-12 #5 actually ran Python 3.11/3.12/3.13 dependency/core compatibility, the merge-blocker counterexamples, the real localhost-HTTP + real-DOCX E2E-12 path, E2E-01 and E2E-09 regression steps; the key steps were not skipped.

This closure adds no second Resident, ResearchAgent, Router, Store, WordAgent, OfficeAgent, Browser/Desktop control plane or frontend feature. It does **not** claim arbitrary DOCX completion, arbitrary document structures, arbitrary internet/Deep Research, authenticated-browser research, universal prompt-injection detection, general citation/reconciliation, or Word/Office Suite complete.
...
The old “first five E2Es to drive orchestration development” ordering is retired because E2E-01/02/03, E2E-09/10, E2E-12, E2E-29, E2E-30/42, E2E-28/34, E2E-27/33, E2E-35 and E2E-36 now have the representative closures recorded above.
...
- Research breadth beyond E2E-01/02/03 and E2E-12, such as authenticated research, PDF/complex-source extraction, citation UX and additional surface combinations;
- Local Office breadth beyond E2E-09/10/12, including complex DOCX/XLSX structures, broader document/spreadsheet transforms, E2E-11 and broader Browser/Research + Office combinations, and other document formats;
