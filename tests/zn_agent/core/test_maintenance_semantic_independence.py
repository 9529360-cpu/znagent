...
class _SingleRouteWorker:
    ...
    def run(self, goal, kernel_context):
        ...
        self.factory.calls.append(
            {"route_id": self.route.route_id, "task": goal.task, "context": kernel_context}
        )
        return WorkerResult(success=True, response=response)


class _SingleRouteFactory:
    def __init__(self, responses):
        self.responses = {key: list(value) for key, value in responses.items()}
        self.calls = []

    def create(self, route):
        return _SingleRouteWorker(self, route)

    def set_health_observer(self, observer):
        return None


class MaintenanceSemanticIndependenceTests(unittest.TestCase):
    @staticmethod
    ...
    def _open_task(resident: CognitiveMaintenanceResidentRuntime) -> dict:
        for _ in range(3):
            resident.health.record_failure(
                "channel:semantic-independence-test",
                AssertionError("semantic-independence-regression"),
            )
        task = resident.health.maintenance_task("channel:semantic-independence-test")
        assert task is not None
        return task

    def test_single_model_route_cannot_self_review_into_automatic_acceptance(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            factory = _SingleRouteFactory(
                {
                    "author": [
                        ...
                        json.dumps(
                            {
                                "decision": "accept",
                                "reason_code": "self_review_accept",
                                "rationale": "this response must never be consumed",
                            }
                        ),
                    ]
                }
            ...
            branch = f"work/self-maintenance-{task['task_id'][:12]}-single-route"
            attempt_root = source_root.parent / ".zn-maintenance-worktrees" / "single-route"
            try:
                ...
                self.assertEqual(result["semantic_review"]["decision"], "unreviewed")
                self.assertEqual(
                    result["semantic_review"]["reason_code"],
                    "independent_route_unavailable",
                )
                self.assertFalse(result["semantic_review"]["independent_route"])
                self.assertEqual(result["investigation"]["status"], "investigating")
                self.assertEqual(result["investigation"]["acceptance_state"], "unreviewed")
                self.assertNotIn("publication_preparation", result)
                self.assertEqual(len(factory.calls), 2)
                self.assertEqual(len(factory.responses["author"]), 1)

                status = resident.status()["maintenance_cognitive_repairs"]
                self.assertEqual(status["dispatch_count"], 2)
                self.assertEqual(status["review_count"], 1)
                self.assertEqual(status["reviews"][0]["decision"], "unreviewed")
                self.assertFalse(status["reviews"][0]["independent_route"])
                self.assertFalse(
                    any(item["phase"] == "semantic_review" for item in status["dispatches"])
                )
            finally:
