...
class OfficeDocumentWorkTests(unittest.TestCase):
    ...
    def test_run_local_and_cross_run_replacement_preserve_run_shape(self) -> None:
        ...
        run = paragraph.add_run("状态：草稿；其他")
        run.bold = True
        start = paragraph.text.index("草稿")
        replace_visible_span_across_runs(paragraph, start, start + len("草稿"), "已确认")
        self.assertEqual(paragraph.text, "状态：已确认；其他")
        self.assertTrue(paragraph.runs[0].bold)

        paragraph = doc.add_paragraph()
        paragraph.add_run("状态：")
        first = paragraph.add_run("草")
        first.bold = True
        second = paragraph.add_run("稿")
        second.italic = True
        start = paragraph.text.index("草稿")
        replace_visible_span_across_runs(paragraph, start, start + len("草稿"), "已确认")
        self.assertEqual(paragraph.text, "状态：已确认")
