...
class MaintenanceSourceInvestigationTests(unittest.TestCase):
    ...
    @staticmethod
    def _make_repo(root: Path, *, origin: str = "https://github.com/9529360-cpu/znagent.git") -> None:
    ...
    def test_rejects_work_subdirectory_foreign_origin_and_non_test_oracle(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "znagent"
            root.mkdir()
            self._make_repo(root)
            ...
            foreign.mkdir()
            self._make_repo(foreign, origin="https://github.com/example/not-zn.git")
            with self.assertRaises(ValueError):
                investigator.investigate(
                    task["task_id"],
                    source_root=foreign,
