...
class ContinuityReferenceProofTests(unittest.TestCase):
    def test_resident_state_proof_covers_memory_will_accounting_and_replay_without_exporting_content(self):
        with tempfile.TemporaryDirectory() as temporary:
            database = Path(temporary) / "kernel.db"
            _init_database(database)
            identity = {"name": "ZN Agent", "version": "0.2.0", "purpose": "private purpose", "principles": ["private principle"], "created_at": "identity-born", "updated_at": "identity-now"}
            living = {"name": "ZN", "version": "0.2.0", "born_at": "living-born", "wake_count": 5, "pulse_count": 10, "last_wake_at": "wake-a", "last_pulse_at": "pulse-a", "mode": "awake", "attention": "volatile attention", "intention": "volatile intention", "drives": [], "observations": ["volatile observation"], "open_questions": ["private question"], "capabilities": [], "external_brains": [], "body": {"pid": 1}, "current_situation": {"sequence": 1}, "current_thought": {"sequence": 1}, "current_impasse": {"impasse_id": "imp-1"}, "learning_candidates": ["learn-1"], "last_event_id": "evt-1", "last_action_summary": "private result"}
            ...
            with sqlite3.connect(database) as conn:
                ...
                conn.execute("INSERT INTO resident_side_effect_attempts VALUES(?,?,?,?,?,?,?,?,?)", ("attempt-1", "evt-1", "hash", "body", "started", "born", None, None, None))
            ...
            self.assertEqual(before["section_counts"]["resident_intentions"], 1)
            self.assertEqual(before["section_counts"]["resident_event_accounting"], 1)
            ...
            with sqlite3.connect(database) as conn:
                ...
                conn.commit()
            after_will_loss = resident_state_proof(database)
            self.assertNotEqual(before["digest"], after_will_loss["digest"])

            with sqlite3.connect(database) as conn:
                conn.execute("INSERT INTO resident_intentions VALUES(?,?,?,?,?)", ("intent-1", "active", 7, "now", json.dumps(intention)))
                conn.execute("DELETE FROM resident_event_accounting WHERE event_id='evt-1'")
                conn.commit()
            after_accounting_loss = resident_state_proof(database)
            self.assertNotEqual(before["digest"], after_accounting_loss["digest"])

    def test_resident_state_proof_ignores_lifecycle_volatility_but_protects_self_anchors(self):
        with tempfile.TemporaryDirectory() as temporary:
            database = Path(temporary) / "kernel.db"
            _init_database(database)
            identity = {"name": "ZN Agent", "version": "0.2.0", "purpose": "continue", "principles": ["preserve continuity"], "created_at": "identity-born", "updated_at": "identity-now"}
            living = {"name": "ZN", "version": "0.2.0", "born_at": "living-born", "wake_count": 5, "pulse_count": 10, "last_wake_at": "wake-a", "last_pulse_at": "pulse-a", "mode": "awake", "attention": "task-a", "intention": "observe", "drives": ["maintain continuity"], "observations": ["old"], "open_questions": ["question-a"], "capabilities": ["cap-a"], "external_brains": ["brain-a"], "body": {"pid": 1}, "current_situation": {"sequence": 10}, "current_thought": {"sequence": 10}, "current_impasse": None, "learning_candidates": ["learn-a"], "last_event_id": "evt-a", "last_action_summary": "summary-a"}
            with sqlite3.connect(database) as conn:
                conn.execute("INSERT INTO identity VALUES(?,?)", (1, json.dumps(identity)))
                conn.execute("INSERT INTO living_self VALUES(?,?,?)", (1, json.dumps(living), "now"))
                conn.commit()
            before = resident_state_proof(database)
            living.update({"wake_count": 6, "pulse_count": 12, "last_wake_at": "wake-b", "last_pulse_at": "pulse-b", "mode": "observing", "attention": None, "intention": "different volatile intention", "observations": ["new"], "capabilities": ["cap-b"], "external_brains": [], "body": {"pid": 2}, "current_situation": {"sequence": 12}, "current_thought": {"sequence": 12}})
            ...
            with sqlite3.connect(database) as conn:
                ...
                conn.commit()
            self.assertEqual(before["digest"], resident_state_proof(database)["digest"])
            living["open_questions"] = []
            with sqlite3.connect(database) as conn:
                conn.execute("UPDATE living_self SET data=? WHERE id=1", (json.dumps(living),))
                conn.commit()
            self.assertNotEqual(before["digest"], resident_state_proof(database)["digest"])

    def test_long_lived_neural_proof_ignores_plasticity_but_detects_retained_memory_loss(self):
        with tempfile.TemporaryDirectory() as temporary:
            database = Path(temporary) / "kernel.db"
            _init_database(database)
            with sqlite3.connect(database) as conn:
                conn.execute("INSERT INTO neural_traces VALUES(?,?,?,?,?,?,?,?)", ("schema-a", "fp-a", "schema", 0.7, 0.8, 1, "t1", '{"summary":"private schema"}'))
                conn.execute("INSERT INTO neural_traces VALUES(?,?,?,?,?,?,?,?)", ("trace-repeat", "fp-b", "world", 0.4, 0.5, 2, "t1", '{"summary":"private repeated memory"}'))
                conn.execute("INSERT INTO neural_traces VALUES(?,?,?,?,?,?,?,?)", ("trace-labile", "fp-c", "world", 0.2, 0.2, 1, "t1", '{"summary":"private labile memory"}'))
                conn.commit()
            before = long_lived_neural_reference_proof(database)
            self.assertEqual(before["count"], 2)
            self.assertNotIn("private schema", repr(before))
            self.assertNotIn("private repeated memory", repr(before))

            with sqlite3.connect(database) as conn:
                conn.execute("UPDATE neural_traces SET strength=?,salience=?,last_seen_at=? WHERE trace_id='schema-a'", (0.99, 0.95, "t2"))
                conn.execute("DELETE FROM neural_traces WHERE trace_id='trace-labile'")
                conn.commit()
            after_plasticity = long_lived_neural_reference_proof(database)
            self.assertEqual(before["digest"], after_plasticity["digest"])

            with sqlite3.connect(database) as conn:
                conn.execute("DELETE FROM neural_traces WHERE trace_id='trace-repeat'")
                conn.commit()
    ...
    def test_exact_work_reference_limit_is_not_reported_as_truncated(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("full_reference_proof", snapshot["long_lived_memory"])
    ...
    def test_changed_resident_or_long_lived_memory_proof_blocks_without_exporting_digest(self):
        for category, key in (("resident_state", "full_state_proof"), ("long_lived_memory", "full_reference_proof")):
            before = _base_snapshot()
            after = _base_snapshot()
            before[category] = {key: {"algorithm": "sha256", "count": 10, "digest": "a" * 64}}
            after[category] = {key: {"algorithm": "sha256", "count": 9, "digest": "b" * 64}}
            verdict = compare_continuity_snapshots(before, after)
            blocker = next(item for item in verdict["blockers"] if item["kind"] == f"{category}_{key}_changed")
    ...
    def test_missing_post_transition_optional_proofs_fail_closed_when_baseline_has_them(self):
        for category, key in (("resident_state", "full_state_proof"), ("long_lived_memory", "full_reference_proof")):
            before = _base_snapshot()
            after = _base_snapshot()
            before[category] = {key: {"algorithm": "sha256", "count": 1, "digest": "a" * 64}}
            verdict = compare_continuity_snapshots(before, after)
            self.assertFalse(verdict["compatible"])
            self.assertIn(f"{category}_continuity_unproven", {item["kind"] for item in verdict["blockers"]})

    def test_old_schema2_baseline_without_new_optional_proofs_remains_compatible(self):
        before = _base_snapshot()
        after = _base_snapshot()
        after["resident_state"] = {"full_state_proof": {"algorithm": "sha256", "count": 1, "digest": "a" * 64}}
        after["long_lived_memory"] = {"full_reference_proof": {"algorithm": "sha256", "count": 1, "digest": "b" * 64}}
        self.assertTrue(compare_continuity_snapshots(before, after)["compatible"])
    ...
    def test_old_schema2_baseline_without_work_proof_uses_bounded_fallback(self):
        before = _base_snapshot()
        after = _base_snapshot()
