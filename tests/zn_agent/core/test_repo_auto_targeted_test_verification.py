...
class RepoTestSemanticsTests(unittest.TestCase):
    def test_canonical_kernel_identity_is_narrow_and_literal(self):
        identity = canonical_kernel_unittest_identity("runtime/python/zn_agent/core/channel.py")
        self.assertEqual(
            identity,
            {
                "kind": "python_unittest",
                "target_relative_path": "runtime/python/zn_agent/core/channel.py",
                "test_relative_path": "tests/zn_agent/core/test_channel.py",
                "target_module": "zn_agent.core.channel",
                "ci_relative_path": ".github/workflows/zn-ci.yml",
            },
        )
        for rejected in (
            "runtime/python/zn_agent/core/__init__.py",
            "runtime/python/zn_agent/core/nested/channel.py",
            "agent/other/channel.py",
            "runtime/python/zn_agent/core/channel.txt",
            "../runtime/python/zn_agent/core/channel.py",
            "C:/repo/runtime/python/zn_agent/core/channel.py",
        ):
    ...
    def test_direct_import_proof_uses_python_structure_not_text_mentions(self):
        ...
        self.assertTrue(
            test_source_directly_imports_target(
                "from zn_agent.core import channel\n",
                module,
            )
        )
        self.assertFalse(
            test_source_directly_imports_target(
                '# from zn_agent.core.channel import X\nVALUE = "zn_agent.core.channel"\n',
                module,
            )
        )
        self.assertFalse(
            test_source_directly_imports_target(
                "from zn_agent.core.channel_runtime import ChannelRuntime\n",
                module,
            )
        )
    ...
    def test_ci_contract_requires_exact_current_kernel_unittest_suite(self):
        current = (
            ...
            "    run: python -m unittest discover -s tests/zn_agent/core "
            "-p 'test_*.py' -v\n"
        )
    ...
    def _write_repo(
        ...
        *,
        ...
        if test_source is not None:
            (root / "tests/zn_agent/core/test_sample.py").write_text(
                test_source,
                encoding="utf-8",
            )
    ...
    def test_resident_forms_and_runs_mirrored_kernel_unittest_from_repo_evidence(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            test_scopes = [
                ...
                == "tests/zn_agent/core/test_sample.py"
            ]
            ci_scopes = [
                item
                for item in actions
    ...
    def test_indirect_test_or_changed_ci_contract_cannot_form_execution_authority(self):
        cases = {
            ...
            "changed-ci": (
                ...
                "name: Test\njobs:\n  kernel:\n    steps:\n"
                "      - run: python -m unittest discover -s tests/agent "
                "-p 'test_*.py' -v\n",
            ),
        }
