...
class OverwriteRecoveryTests(unittest.TestCase):
    ...
    @staticmethod
    def _crash_before_overwrite_dispatch(resident):
        original = resident.body.act

        def crash_before_dispatch(kind: str, *, event_id=None, **args):
            normalized = str(kind or "").strip().lower()
            if normalized in {"write_text", "write_file"} and not bool(args.get("append", False)):
                raise SystemExit("simulate process death before overwrite Body dispatch")
            return original(kind, event_id=event_id, **args)
    ...
    def test_restart_observes_completed_overwrite_and_never_replays_it(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                prestate = persisted.data["overwrite_prestate_identity"]
                identity = prestate["identity"]
                self.assertEqual(identity["type"], "file")
                self.assertTrue(identity["digest_complete"])
                self.assertEqual(len(identity["content_sha256"]), 64)
                self.assertNotIn("old", str(identity["content_sha256"]))
            finally:
            ...
            try:
                ...
                self.assertTrue(recovery["prestate_checkpointed"])
    ...
    def test_restart_refuses_overwrite_when_current_file_changed_after_crash(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertFalse(recovery["prestate_recheck"]["exact"])
    ...
    def test_restart_blocks_predispatch_overwrite_when_target_drifted(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store_path = root / "kernel.db"
            target = root / "document.txt"
            target.write_text("old baseline", encoding="utf-8")
            intended = "new resident state"
            external = "user changed before restart"
            ...
            event, state = self._prepare_overwrite(resident, target, intended)
            self._crash_before_overwrite_dispatch(resident)
            try:
                with self.assertRaises(SystemExit):
                    resident._native_action_step(event, state, readiness=None)
                persisted = resident.store.get_working_state()
                self.assertIn("overwrite_prestate_identity", persisted.data)
                self.assertEqual(self._attempts(store_path, event.event_id), [])
                self.assertEqual(target.read_text(encoding="utf-8"), "old baseline")
            finally:
                resident.store.close()
            ...
            try:
                ...
                self.assertIn("pre-dispatch identity", held.data["local_failure"])
                self.assertFalse(held.data["overwrite_prestate_recheck"]["exact"])
                self.assertEqual(target.read_text(encoding="utf-8"), external)
                self.assertEqual(self._attempts(store_path, event.event_id), [])
                writes = [
                    item
    ...
    def test_restart_resumes_predispatch_overwrite_only_when_exact_prestate_remains(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            store_path = root / "kernel.db"
            target = root / "document.txt"
            ...
            try:
                ...
                self.assertEqual(target.read_text(encoding="utf-8"), intended)
                after_action = restored.store.get_working_state()
                self.assertTrue(after_action.data["overwrite_prestate_recheck"]["exact"])
                self.assertEqual(after_action.stage, "native_verification")
                attempts = self._attempts(store_path, event.event_id)
                self.assertEqual(len(attempts), 1)
                self.assertEqual(attempts[0][1], "observed")

                result = restored._native_verification_step(
                    restored_event,
                    after_action,
                    readiness=None,
                )
