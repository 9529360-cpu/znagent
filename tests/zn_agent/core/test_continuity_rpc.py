    ...
    def test_formal_resident_compares_baseline_against_current_subject_without_mutation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertEqual(baseline["atomic_overwrite_recovery"]["version"], 1)
                self.assertEqual(
                    baseline["atomic_overwrite_recovery"]["algorithm"], "sha256"
                )
                self.assertEqual(
                    baseline["atomic_overwrite_recovery"]["protocols"], []
                )
                ...
                self.assertIn(
                    "atomic_overwrite_recovery",
                    compare_response["result"]["current"],
                )
            finally:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                first.nervous.perceive(
                    "world", "persistent continuity cue", features=("continuity",)
                )
                first.nervous.perceive(
                    "world", "persistent continuity cue", features=("continuity",)
                )
            ...
            try:
                ...
                current_resident_hashes = set(
                    response["result"]["current"]["resident_state"][
                        "full_state_proof"
                    ]["reference_hashes"]
                )
                ...
                self.assertTrue(
                    baseline_resident_hashes.issubset(current_resident_hashes)
                )

                current_memory_hashes = set(
                    response["result"]["current"]["long_lived_memory"][
                ...
                self.assertTrue(
                    baseline_memory_hashes.issubset(current_memory_hashes)
                )
                self.assertEqual(
                    response["result"]["current"]["atomic_overwrite_recovery"][
                        "version"
                    ],
                    1,
                )
