...
<body><main>
  ...
  <form id="note-form" action="/orders/alice?saved=1" method="post">
  ...
  const timer = setInterval(async () => {{
    try {{
      ...
      if ((await response.text()) === 'changed') {{
        ...
        if (!location.search) document.title = '{_FRESH_TITLE}';
        clearInterval(timer);
      }}
    }} catch {{}}
  }}, 80);
    ...
    def do_POST(self) -> None:
        ...
        values = parse_qs(raw, keep_blank_values=True).get("note", [])
        note = values[0] if len(values) == 1 else ""
...
class E2E05AuthenticatedResearchNoteMutationTests(unittest.TestCase):
    @staticmethod
    def _require_input_desktop() -> None:
        WindowsInteractiveUserBrowserBridgeProviderE2ETests._require_input_desktop()

    def test_existing_session_public_research_returns_to_exact_tab_and_persists_note(self) -> None:
        ...
        try:
            ...
            thread_id = "e2e05-authenticated-order-note"
            created = rpc.handle({"id": "create", "method": "work_create", "params": {"thread_id": thread_id, "title": "Alice return note"}})
            ...
            self.assertIsNotNone(final, "E2E-05 Work did not reach a terminal result")
            ...
            while time.monotonic() < deadline:
                ...
                if _SAVED_TITLE.lower() in candidate.title.lower():
                    ...
                    break
                time.sleep(0.05)
            self.assertIsNotNone(foreground, "fresh Windows evidence did not prove the persisted saved-note page")
            ...
            print(
                "ZN_E2E05_AUTH_RESEARCH_MUTATION="
                + json.dumps(
                    {
                        "provider": provider,
                        "normal_language_task": _TASK,
                        "existing_authenticated_user_session": True,
                        "same_authorization_generation_through_mutation": True,
                        "managed_received_user_cookie": False,
                        "fresh_page_changed_during_research": True,
                        "saved_note": policy,
                        ...
                        "independent_verifiers": ["http-server-persisted-state", "windows-foreground"],
                    },
                    ensure_ascii=False,
                    sort_keys=True,
                ),
