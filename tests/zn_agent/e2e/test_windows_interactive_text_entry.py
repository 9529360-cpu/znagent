...
class _Win32EditFixture:
    TITLE = "ZN Interactive Text Entry E2E"
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="zn-interactive-text-e2e-window",
            daemon=True,
        )
...
class WindowsInteractiveTextEntryE2ETests(unittest.TestCase):
    @staticmethod
    def _require_input_desktop() -> None:
        if os.name != "nt":
            raise unittest.SkipTest("Windows interactive text E2E runs only on Windows")
        user32 = ctypes.WinDLL("user32", use_last_error=True)
        ...
        if not desktop:
            raise AssertionError(
                ...
                "interactive text E2E requires an unlocked interactive user session"
            )
        try:
