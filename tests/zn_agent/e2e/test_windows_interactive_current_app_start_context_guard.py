...
class WindowsInteractiveCurrentAppStartContextGuardE2ETests(unittest.TestCase):
    ...
    def test_session_drift_after_grounding_refuses_setvalue_before_side_effect_attempt(self) -> None:
        require_input_desktop()
        with tempfile.TemporaryDirectory(
            prefix="zn-e2e13-session-toctou-",
            ignore_cleanup_errors=True,
        ) as tmp:
            root = Path(tmp)
            app_root = root / "app"
            app_root.mkdir()
            app = WorkRecordApp(app_root)
            resident = None
            try:
                app.start()
                resident = build_resident_runtime(
                ...
                control = RestoreAwareWorkControl(RecoveryBoundedWorkLedger(resident))
                thread = "e2e13-session-" + uuid.uuid4().hex[:8]
                control.ledger.create_thread(thread_id=thread)
                ...
                self.assertIsInstance(start_context, dict)
                assert isinstance(start_context, dict)
                start_session = start_context.get("components", {}).get("session")
                self.assertIsInstance(start_session, str)
                ...
                while time.monotonic() < deadline and not grounded:
                    ...
                    if current is not None and current.event.event_id == event.event_id:
                        self.fail(
                            "E2E-13 terminated before reaching the replacement stage: "
                            + str(current.reason or current.response or "")
                        )
                    state = resident.store.get_working_state()
                    if state.current_event_id == event.event_id:
                        trace.append({"stage": state.stage, "next_action": state.next_action})
                        grounded = state.stage == "e2e13_replace"
                    if not grounded:
                        time.sleep(0.03)

                self.assertTrue(grounded, json.dumps(trace[-20:], ensure_ascii=False))
                baseline = resident.device_capabilities.companion_frame()
                self.assertEqual(baseline.session_fingerprint, start_session)
                self.assertEqual(resident.body.value_replacement_attempts(event.event_id), [])
                before_actions = [
                    action
                    for action in resident.body.recent_actions(1024)
                    if action.event_id == event.event_id
                ...
                self.assertFalse(
                    any(action.kind == "automation_value_replace" for action in before_actions)
                )
                ...
                drifted = replace(
                    ...
                    session_connection_state="disconnected",
                )
                resident.device_capabilities.companion_frame = lambda: drifted
                ...
                assert result is not None
                self.assertFalse(result.success)
                self.assertIn("session changed", str(result.reason or "").lower())
                self.assertIn("valuepattern", str(result.reason or "").lower())
                ...
                print(
                    "ZN_WINDOWS_CURRENT_APP_SESSION_TOCTOU_EVIDENCE="
                    f"{{\"event_id\":\"{event.event_id}\","
                    "\"disposition\":\"session_drift\","
                    "\"replacement_attempt_count\":0,"
                    f"\"mutation_count\":{sum(action.kind == 'automation_value_replace' for action in actions)},"
                    f"\"save_click_count\":{sum(action.kind == 'pointer_click' for action in actions)},"
                    f"\"save_count\":{app.saved_count()}}}",
                    flush=True,
                )
            finally:
                app.close()
