...
from test_windows_interactive_desktop_semantic_grounding import (
    ...
    SUCCESS_TITLE,
    TASK,
    WindowsInteractiveDesktopSemanticGroundingContractTests,
    _GroundingOrderApp,
)
from test_windows_interactive_text_entry import WindowsInteractiveTextEntryContractTests
...
class WindowsInteractiveDesktopSemanticRegroundContractTests(unittest.TestCase):
    def test_runtime_label_drift_is_resensed_semantically_then_completed(self) -> None:
        WindowsInteractiveTextEntryContractTests._require_input_desktop()
        with tempfile.TemporaryDirectory() as tmp:
            ...
            WindowsInteractiveDesktopSemanticGroundingContractTests._stamp_yesterday(source)
            ...
            try:
                ...
                WindowsInteractiveDesktopSemanticGroundingContractTests._enable_cognition(
                    resident,
                    proposal,
                )
                ledger = RecoveryBoundedWorkLedger(resident)
                thread = "desktop-semantic-reground-contract"
                ...
                print(
                    "ZN_DESKTOP_SEMANTIC_REGROUND_Contract_EVIDENCE="
                    + json.dumps(
                        {
