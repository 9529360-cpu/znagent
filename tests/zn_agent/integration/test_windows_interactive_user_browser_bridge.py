...
class _IsolatedUserBrowserFixture:
    ...
    def __init__(
        ...
        *,
        ...
        self.root = Path(tempfile.mkdtemp(prefix=f"zn-{self.provider}-bridge-contract-"))
...
class WindowsInteractiveUserBrowserBridgeProviderContractTests(unittest.TestCase):
    @staticmethod
    def _require_input_desktop() -> None:
        if os.name != "nt":
            raise unittest.SkipTest("Windows user-browser bridge Contract runs only on Windows")
        user32 = ctypes.WinDLL("user32", use_last_error=True)
