...
from test_windows_interactive_user_browser_bridge import (
    ...
    WindowsInteractiveUserBrowserBridgeProviderContractTests,
)
...
_HOST = "zn-extension-contract.test"
_TITLE = "ZN User Browser Bridge Contract"
_RESULT_TITLE = "ZN Customer Order Result Contract"
...
class WindowsInteractiveUserBrowserSemanticGroundingContractTests(unittest.TestCase):
    @staticmethod
    def _require_input_desktop() -> None:
        WindowsInteractiveUserBrowserBridgeProviderContractTests._require_input_desktop()
    ...
    def _enable_cognition(resident, cognition: _SemanticBrowserCognition) -> None:
        resident.kernel.reconfigure_resources(
            routes=[
                ModelRoute(
                    route_id="contract-user-browser-semantic-grounding",
                    provider="fixture",
                    model="bounded-language-fixture",
    ...
    def test_ordinary_goal_regrounds_after_semantic_label_and_node_drift_then_verifies_result(self) -> None:
        ...
        try:
            ...
            self.assertEqual(result.response, f"{_TEXT}: {_STATUS}")
            print("ZN_USER_BROWSER_SEMANTIC_GROUNDING_Contract_EVIDENCE=" + json.dumps({
