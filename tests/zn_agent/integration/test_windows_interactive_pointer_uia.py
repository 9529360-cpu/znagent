...
class _Win32FocusFixture:
    ...
    TITLE = "ZN Interactive Desktop Contract"
    ...
    def __init__(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="zn-interactive-contract-window",
            daemon=True,
        )
    ...
    def target_center(self) -> tuple[int, int]:
        ...
        if not user32.GetWindowRect(self.target_hwnd, ctypes.byref(rect)):
            raise OSError("GetWindowRect failed for the Contract target")
        return (
            int(rect.left) + 16,
...
class WindowsInteractivePointerUiAContractTests(unittest.TestCase):
    @staticmethod
    def _require_input_desktop() -> None:
        if os.name != "nt":
            raise unittest.SkipTest("Windows interactive desktop Contract runs only on Windows")
        user32 = ctypes.WinDLL("user32", use_last_error=True)
        ...
        if not desktop:
            raise AssertionError(
                ...
                "interactive Contract requires an unlocked interactive user session"
            )
        try:
        ...
        if not user32.GetCursorPos(ctypes.byref(original)):
            raise AssertionError("interactive Contract cannot read the real Windows cursor")
        ...
        try:
            if not user32.SetCursorPos(probe_x, probe_y):
                raise AssertionError(
                    "Windows input desktop rejects SetCursorPos; runner is not authoritative for input Contract"
                )
            if not user32.GetCursorPos(ctypes.byref(probe)):
                raise AssertionError("interactive Contract cannot re-read the cursor after SetCursorPos")
