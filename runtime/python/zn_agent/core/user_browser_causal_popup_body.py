...
"""Bounded USER-browser causal child-tab Body movement."""
...
class UserBrowserCausalPopupBody(BrowserFormSubmitBody):
    """Extend the exact button action with one proven USER child tab."""
    ...
    def _dispatch(self, action: BodyAction, started: str) -> BodyActionResult:
        if (
            action.kind == self._BROWSER_CLICK_NAMED_BUTTON
            and action.args.get("causal_popup_allowed") is True
        ):
            return self._browser_click_named_button_with_causal_child(action, started)
    ...
    def _browser_click_named_button_with_causal_child(
        self,
        action: BodyAction,
        started: str,
    ) -> BodyActionResult:
        ...
        target_name = self._bounded_label(
        ...
        try:
            ...
            if evidence.postcondition != self._CAUSAL_POSTCONDITION:
                raise ValueError("causal child click returned an unsupported completion contract")

            # Child authority has been released. Re-observe through the normal root
            # adapter so popup-pre target evidence cannot cross this interruption.
            ...
            if not root_ok:
                ...
                return BodyActionResult(
                    ...
                    error=(
                        "fresh root re-sense did not preserve the exact Work authorization context"
                    ),
                    event_id=action.event_id,
                    started_at=started,
                    completed_at=utc_now(),
            ...
            return BodyActionResult(
                ...
                data={
                    ...
                    "postcondition": self._CAUSAL_POSTCONDITION,
                    "target_revalidated_before_dispatch": bool(
                    ...
                    "child_authority_task_scoped": (
                        data.get("child_authority_task_scoped") is True
                    ),
                    "child_debugger_detached": data.get("child_debugger_detached") is True,
                    "root_authorization_preserved": (
                        data.get("root_authorization_preserved") is True
                    ),
                    "root_generation_unchanged": (
                        data.get("root_generation_unchanged") is True
                    ),
                    "returned_to_exact_root_tab": (
                        data.get("returned_to_exact_root_tab") is True
                    ),
