...
A frame is not a new world-state authority. It is a compact identity of the
existing fresh DeviceCapabilityGraph observations so Work/Situation/learning can
bind a decision to the Windows context that actually existed without persisting
raw foreground title text, network addresses or display hardware identifiers.
"""
...
class WindowsCompanionFrameSense:
    """Compose one fresh frame from the existing Resident DeviceCapabilityGraph."""
    ...
    def probe(self) -> WindowsCompanionFrame:
        ...
        power_payload = {
            ...
            # Percentage/lifetime intentionally excluded from identity: routine
            # battery drain must not invalidate a context frame every minute.
        }
        network_payload = {
            ...
            # Interface names, addresses and adapter counts are not identity.
        }
        display_payload = {
        ...
        return WindowsCompanionFrame(
            ...
            foreground_process_id=(
                None if foreground is None else foreground.process_id
            ),
            foreground_process_name=(
                None if foreground is None else foreground.process_name
            ),
...
def frame_to_bounded_evidence(frame: WindowsCompanionFrame) -> dict[str, Any]:
    """Return the compact public evidence shape, excluding component payloads."""

    data = asdict(frame)
    # The fingerprints prove current context identity; raw component payloads
    # were never stored in the frame in the first place.
    return data
