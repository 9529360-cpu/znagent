...
class ResidentAccountingJournal:
    """Commit self evidence and runtime metrics once for one durable event fact.

    Capability execution is journaled before this accounting runs. If the process
    dies after this transaction but before ``resident_completion`` is saved, a
    restart may safely call the same method again: the event/kind key makes the
    second call a no-op rather than duplicating self-model evidence or task totals.
    """
