...
_FORBIDDEN_AUTHORITY_FIELDS = frozenset(
    {
        "runtime_id", "process_id", "process_name", "selector", "automation_id",
        "coordinates", "x_fraction", "y_fraction", "completion_verified", "authorized",
    }
)
...
_NAMED_DESTINATION_PATTERNS = (
    re.compile(r"(?:找到|使用|选择)?\s*(?P<role>输入框|文本框|字段)\s*[\"“'‘](?P<name>[^\"”'’‘，,。；;\r\n]{1,160})[\"”'’]"),
    re.compile(r"[\"“'‘](?P<name>[^\"”'’‘，,。；;\r\n]{1,160})[\"”'’]\s*(?P<role>输入框|文本框|字段)"),
)
...
_VALUE_BEFORE_DESTINATION_RE = re.compile(r"把\s*(?P<name>[^，,。；;\r\n]{1,64}?)\s*(?:填|输入|写|放)(?:到|进|入)?")
_SOURCE_HINT_RE = re.compile(
    r"名字(?:里)?(?:像|包含|带有|带)\s*[\"“'‘]?(?P<hint>[^\"”'’‘，,。；;\r\n]{1,64}?)[\"”'’]?\s*"
    r"的(?:那个|那份|那个文件|文件)?\s*\.?\s*txt(?=[，,。；;\s]|$)", re.I,
)
_YESTERDAY_HINT_RE = re.compile(r"昨天(?:那份|那个|的)?(?P<hint>[A-Za-z0-9_\-\u4e00-\u9fff]{1,32}?)(?:资料|文件|txt)")
...
def desktop_task_request(event: "AgentEvent") -> dict[str, Any] | None:
    ...
    if str(event.kind or "").strip().lower() != "desktop_user_event" or payload.get("body_action") or payload.get("native_action"):
        return None
    ...
    target_name = _input_semantic_name(_clean_semantic(destination["name"]), destination["role"], value_name=value_name)
    target_binding = "exact_name" if target_name else "focused_safe_edit"
    fill_matches = list(_FILL_CUE_RE.finditer(task))
    tail_start = max(int(destination["end"]), max((m.end() for m in fill_matches), default=int(destination["end"])))
    submit_name = _submit_name(task[tail_start:])
    final_title = _final_title(task)
    if final_title and not submit_name:
        return None
    expected_final_state = (
        ({"kind": "foreground_title_equals", "title": final_title} if final_title else {"kind": "foreground_title_changed"})
        if submit_name else {"kind": "text_matches"}
    )
    return {
...
def _natural_source_requirement(task: str, payload: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if workspace and any(token in task for token in ("文件", "资料", "txt")):
        ...
        return {"kind": "workspace_file", "workspace_path": workspace, "name_hint": hint, "modified_day": "yesterday" if "昨天" in task else None, "value_semantic_name": _value_semantic_name(task) or "file_text"}
    lowered = task.lower()
    current_page = any(c in lowered for c in ("current browser page", "current page", "authorized page")) or any(c in task for c in ("当前浏览器页面", "当前页面", "已授权的当前浏览器", "已授权页面"))
    references = "reference" in lowered or "参考" in task
    agreement = any(c in lowered for c in ("agree", "same", "consistent")) or any(c in task for c in ("一致", "相同", "共同"))
    release_code = "release code" in lowered or ("release" in lowered and "code" in lowered) or "发布代码" in task or "发行代码" in task
    if current_page and references and agreement and release_code:
        return {"kind": "managed_browser_research", "value_semantic_name": "release code", "require_two_sources": True}
    return None
...
def _natural_destination(task: str) -> dict[str, Any] | None:
    ...
    if direct is not None:
        return {"name": direct.group("name"), "role": direct.group("role"), "start": direct.start(), "end": direct.end()}
    if not _FILL_CUE_RE.search(task):
        return None
    matches: list[dict[str, Any]] = []
    for pattern in _NAMED_DESTINATION_PATTERNS:
        for match in pattern.finditer(task):
            item = {"name": match.group("name"), "role": match.group("role"), "start": match.start(), "end": match.end()}
            if item not in matches:
                matches.append(item)
    return matches[0] if len(matches) == 1 else None


def _validated_proposal(task: str, payload: Mapping[str, Any], proposed: Mapping[str, Any]) -> dict[str, Any] | None:
    if any(key in proposed for key in _FORBIDDEN_AUTHORITY_FIELDS):
        return None
    if str(proposed.get("kind") or "").strip() not in {"desktop_value_task", "desktop_value_submit"}:
        return None
    source, app_scope, final = proposed.get("source_requirement"), proposed.get("destination_app_scope"), proposed.get("expected_final_state")
    if not isinstance(source, Mapping) or not isinstance(app_scope, Mapping) or not isinstance(final, Mapping):
        return None
    if any(key in source for key in _FORBIDDEN_AUTHORITY_FIELDS) or any(key in app_scope for key in _FORBIDDEN_AUTHORITY_FIELDS) or any(key in final for key in _FORBIDDEN_AUTHORITY_FIELDS):
        return None
    source_kind = str(source.get("kind") or "").strip()
    if source_kind not in {"workspace_file", "managed_browser_research"} or str(app_scope.get("kind") or "").strip() != "current_foreground_non_browser":
        return None
    ...
    if source_kind == "workspace_file":
        ...
        if modified_day not in {None, "yesterday"} or (modified_day == "yesterday" and "昨天" not in task):
            return None
        ...
        if value_name != "file_text" and not _semantic_grounded_in_task(task, value_name):
            return None
        normalized_source = {"kind": "workspace_file", "workspace_path": workspace, "name_hint": hint, "modified_day": modified_day, "value_semantic_name": value_name}
    else:
        ...
        if not result_name or not _semantic_grounded_in_task(task, result_name):
            return None
        normalized_source = {"kind": "managed_browser_research", "value_semantic_name": result_name, "require_two_sources": True}
    normalized_final = {"kind": final_kind}
    if final_title:
        normalized_final["title"] = final_title
    return {"kind": "desktop_value_task", "source_requirement": normalized_source, "destination_app_scope": {"kind": "current_foreground_non_browser"}, "target_input_binding": binding, "target_input_semantic_name": target_name, "submit_control_semantic_name": submit_name, "expected_final_state": normalized_final}
...
def _value_semantic_name(task: str) -> str:
    ...
    for prefix in ("文件里的", "文件中的", "文件里", "资料里的", "资料中的", "资料里", "里面的", "查到的", "确认的"):
...
def _traceable_semantic(task: str, value: str) -> bool:
    return bool((needle := "".join(str(value or "").split()).casefold()) and needle in "".join(str(task or "").split()).casefold())
