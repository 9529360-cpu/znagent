...
legitimately add state or advance mutable statuses while every protected
baseline entity/content anchor must still survive. This removes the old
100/256 proof ceiling without freezing a living resident during verification.
"""
...
class ContinuitySnapshotService:
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "work": {
                ...
                "references_may_be_truncated": thread_count > len(threads),
    ...
    def _verified_learning_snapshot(self) -> dict[str, Any]:
        ...
        if int(proof["count"]) != total:
            raise RuntimeError("verified learning continuity count changed while snapshotting")
        return {
    ...
    def _provider_snapshot(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "credential": {
                ...
                "environment_name": str(environment_name) if environment_name is not None else None,
            },
...
def compare_continuity_snapshots(before: Mapping[str, Any], after: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if before_schema != after_schema or before_schema != ContinuitySnapshotService.SCHEMA_VERSION:
        blockers.append({"kind": "schema_mismatch", "before": before_schema, "after": after_schema})
    ...
    _require_proof_survival(
        ...
        before=_mapping(before.get("long_lived_memory")).get("full_reference_proof"),
...
def _require_equal(blockers: list[dict[str, Any]], kind: str, before: Mapping[str, Any], after: Mapping[str, Any], key: str) -> None:
    old = before.get(key)
    new = after.get(key)
...
def _require_non_decreasing_counter(blockers: list[dict[str, Any]], kind: str, before: Mapping[str, Any], after: Mapping[str, Any], key: str) -> None:
    try:
...
def _require_id_survival(blockers: list[dict[str, Any]], *, category: str, before: Any, after: Any) -> None:
    ...
    if missing:
        blockers.append({"kind": f"{category}_references_lost", "missing_reference_count": len(missing)})
...
def _require_proof_survival(
    ...
    *,
    ...
    if old is None or new is None:
        blockers.append({"kind": f"{category}_continuity_unproven", "reason": f"full {proof_label} is invalid or missing"})
        return
    old_hashes = old["hashes"]
    new_hashes = new["hashes"]
    if old_hashes is not None:
        if new_hashes is None:
            blockers.append({"kind": f"{category}_continuity_unproven", "reason": f"full {proof_label} lacks reference hashes"})
            return
...
def _require_reference_survival(
    ...
    *,
    ...
    if bool(before.get("references_may_be_truncated")):
        blockers.append({"kind": f"{category}_baseline_incomplete", "reason": "pre-transition reference set is truncated"})
    ...
    if bool(after.get("references_may_be_truncated")):
        blockers.append({"kind": f"{category}_continuity_unproven", "missing_reference_count": len(missing), "reason": "post-transition reference set is truncated"})
        return
    blockers.append({"kind": f"{category}_references_lost", "missing_reference_count": len(missing)})
